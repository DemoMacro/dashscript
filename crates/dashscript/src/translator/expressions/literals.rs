//! Value-literal renderers: `.ts` string / number / boolean → `syn::Expr`.

use oxc_ast::ast::StringLiteral;
use proc_macro2::Span;
use syn::{parse_quote, parse_str, Expr};

/// `.ts` string literal → Rust `String` (`"…".to_string()`).
...
///
/// A `.ts` `number` is an IEEE-754 double — i.e. Rust `f64`. Every literal is
