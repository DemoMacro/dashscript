...
fn engine_eval(js_source: &str, includes: &[String]) -> EngineOutcome {
    ...
    // Returns `Ok(None)` on a clean run; `Ok(Some(msg))` if the fixture threw
    // (the thrown value stringified by the wrapper — "Test262Error: …" or
    // "ReferenceError: Temporal is not defined"); `Err` if a prelude/harness
    // eval itself failed. Fetching the thrown value via `ctx.catch()` rather
    // than formatting the `rquickjs::Error` is what makes engine-path failures
    // diagnosable — that Error's Display is the opaque "Exception generated by
    // QuickJS", which hid ~4000 fixtures' real cause behind a fixed string.
    let result = ctx.with(
    ...
    match result {
        ...
        Err(e) => EngineOutcome::OtherError(format!("harness eval: {e}")),
    }
}
