...
for (const bench of benches) {
  ...
  const row = { bench, name, times: {}, checksums: {}, error: {} };

  for (const rt of RUNTIMES) {
    try {
      rt.build(dir, name);
      const cmd = rt.runCmd(dir, name);
      const out = shOut(cmd, { cwd: dir });
      const checksum = out.trim().split(/\r?\n/).pop().trim();
      row.checksums[rt.name] = checksum;
      row.times[rt.name] = timeRun(cmd, dir);
    } catch (e) {
      row.times[rt.name] = null;
      row.error[rt.name] = String(e).split("\n")[0].slice(0, 100);
    }
  }

  const okVals = Object.entries(row.checksums).map(([k, v]) => v);
  ...
  console.log(
    [
      bench.padEnd(12),
      ...RUNTIMES.map((r) => fmt(row.times[r.name])),
