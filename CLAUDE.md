...
**`manifest.json` — the package.json ∩ Cargo.toml intersection (vs `package.json`).**
A `package.json` at the project root is claimed by npm/pnpm and would mislead JS tooling. A dedicated `manifest.json` avoids the collision and is the **semantic intersection of `package.json` and `Cargo.toml`** — TS users write the package.json fields they know, DashScript emits the matching cargo idioms: metadata (`name`/`version`/`description`/`license`/`repository`/`homepage`/`keywords`/`authors` → `[package]`); `bin` (package.json `bin`, string or object → cargo `[[bin]]`); `lib` (`main`/`module` → `[lib]`); `dependencies`/`devDependencies` (rust:-prefixed → `[dependencies]`/`[dev-dependencies]`); `scripts` (package.json-only, no cargo analogue). Plus DashScript-specific `target` (output shape: `bin` default / `rust` / `wasm` / `napi`) and `workspaces` member globs (plural, mirroring npm/yarn/bun's `package.json` field; pnpm alone uses a separate `pnpm-workspace.yaml`). ✅ one file = one crate, every entry a real cargo target · ❌ one more file format to document.
