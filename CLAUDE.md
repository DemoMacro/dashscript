...
- **Compatibility — degrade, don't reject** — a construct the static translator cannot lower runs under an embedded QuickJS engine at the **function** granularity (inheriting full ECMAScript semantics) instead of failing, so existing TS/JS keeps working. This is also the WinterTC (Ecma TC55) compatibility path: the ECMAScript core via the engine, Web APIs via Rust crates. _Today: per-function and transitive whole-module (npm `.js`) degradation are in via embedded QuickJS; whole-program degradation remains for the conformance oracle._
