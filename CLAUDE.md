...
The harness runs each tc39 test262 fixture — the official ECMAScript suite Node/Bun/Deno/V8 all run — and the verdict is **assert-driven**: the fixture's own `assert.sameValue(a, b)` / `assert.throws(…)` carry the expected values (the ECMAScript reference), so the result is whether every assert held — never a stdout diff against a Node oracle (Node ≠ spec; the `expected` arg _is_ the spec truth). **Every fixture goes through the compile path** — `Translator::check` → `translate` + `cargo build` → run the produced native binary — there is no separate in-process engine testbed. A fixture the static translator cannot lower degrades per-function: the function body is emitted into the production binary's embedded QuickJS (`__ds_engine` + `wire_web_apis`), so the degraded body runs inside the same binary the user ships, never a test harness. Test compatibility (the test262 `assert`/`throws`/`Test262Error`, WPT `assert_equals`/`AssertionError`, `console`) is lifted into production as `register_*` engine builtins — the Javy register pattern, where one Rust impl is shared by the static `__ds::X` path and the engine builtin (no test-only shim, no injected `sta.js`/`assert.js`); reflection (`verifyProperty`/property descriptors/prototype chains) likewise runs with reference semantics inside that embedded QuickJS — "degrade, don't reject" applied to conformance. Verdict: exit 0 = `supported`, a thrown `Test262Error`/`AssertionError` = `partial`, a build failure / timeout / `ReferenceError` (the binary's QuickJS lacks a host the translator cannot provide — the `$262` agent API for true threads, `$DONE` for an async event loop, ShadowRealm reflection) = `unsupported`. The extractor (`scripts/extract-test262.mjs`) carries every `test/built-ins/<cat>/` fixture + its `includes:` list (no whitelist); constructs the static translator cannot lower degrade per-function rather than being filtered out. ✅ no hand-written oracle, the spec's own expected values, no test-only testbed — conformance measures the production binary · ❌ the TS→Rust subset means much of test262 is `partial` / `unsupported` today — that backlog is the output, surfaced honestly. (`bcd`/`runtime-compat` were dropped: they test API _existence_, not _semantics_; a Node oracle was dropped: render-parity with Node ≠ spec-conformance.)
